 ...
 */
@Controller
public class CHtoENController {

    // OTS webapi 接口地址
    private static final String WebITS_URL = "https://itrans.xfyun.cn/v2/its";
    // 应用ID（到控制台获取）
    private static final String APPID = "6044d3ca";
    // 接口APISercet（到控制台机器翻译服务页面获取）
    private static final String API_SECRET = "03397ea9352f7b867ac0566e45015653";
    // 接口APIKey（到控制台机器翻译服务页面获取）
    private static final String API_KEY = "df5812cf22687d807d62f3bc5f9489e4";
    ...
    @PostMapping("/jjfy")
    @ResponseBody
    public JsonData jjfy(@RequestBody String param){
        String resultStr = "";
        try {
            JSONObject jsonObject = JSON.parseObject(param);
            TEXT = jsonObject.get("param").toString();
            TO = jsonObject.get("languge").toString();
            if (APPID.equals("") || API_KEY.equals("") || API_SECRET.equals("")) {
                System.out.println("Appid 或APIKey 或APISecret 为空！请打开demo代码，填写相关信息。");
                return JsonData.fail("Appid 或APIKey 或APISecret 为空！请打开demo代码，填写相关信息。");
            }
            ...
            if (resultMap != null) {
                String resultStr1 = resultMap.get("body").toString();
                System.out.println("【ITS WebAPI 接口调用结果】\n" + resultStr1);
                //以下仅用于调试
                Gson json = new Gson();
                ResponseData resultData = json.fromJson(resultStr1, ResponseData.class);
                int code = resultData.getCode();
                Map data = (Map)resultData.getData();
                Map result = (Map)data.get("result");
                Map trans_result = (Map)result.get("trans_result");
                resultStr = trans_result.get("dst").toString();
        ...
        return JsonData.success(resultStr);
    }
